...
const ChatHistorySidebar: React.FC<ChatHistorySidebarProps> = ({
  ...
  return (
    <div className={cn(
      "fixed left-16 top-18 h-[calc(100vh-72px)] z-30 flex flex-col",
      isCollapsed 
        ? "w-16 bg-transparent border-r-transparent" 
        : "w-64 bg-gr-25 border-r border-n-75",
      className
    )}>
      {/* Header */}
      <div className={cn(
        "p-4",
        !isCollapsed && "border-b border-n-75"
      )}>
        <div className="flex items-center justify-between mb-4">
          <button
            ...
            title={isCollapsed ? "Expand chat history" : "Collapse chat history"}
          >
            {isCollapsed ? (
